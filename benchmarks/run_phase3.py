...
class Phase3Evaluator:
    ...
    def __init__(
        ...
        logger.info("Phase 3 Evaluator initialized (Long-Video Stress Test)")
    ...
    def evaluate_sample(
        ...
        # Compute metrics
        # For long videos, frame count scales with duration
        video_duration = sample.video_duration_sec or 60.0
        fps = config.frame_sampling.target_fps

        # Actual frames would be min(duration * fps, max_frames)
        actual_frames = min(
            int(video_duration * fps),
            config.frame_sampling.max_frames,
        )

        # Token usage scales with frame count and context
        input_tokens = 1024 + (actual_frames * 32)  # Base + per-frame tokens
        output_tokens = 128

        # Add retrieval time for configs with timeline/KB
        if config.retrieval.use_timeline or config.retrieval.use_kb:
            retrieval_time = 0.1 + (video_duration / 60) * 0.05  # Scales with duration
    ...
    def _run_inference(
        ...
        """
        Run model inference on a long-video sample.
        ...
        """
        # Placeholder
        return "A" if sample.ground_truth else "B"
