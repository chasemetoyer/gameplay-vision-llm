...
from benchmarks.loaders.videogameqa import VIDEOGAMEQA_TASKS
...
class Phase2Evaluator:
    ...
    def __init__(
        ...
        self.available_tasks = list(VIDEOGAMEQA_TASKS.keys())

        logger.info("Phase 2 Evaluator initialized")
    ...
    def _run_inference(
        ...
        """
        # Placeholder response based on task type
        if sample.task_type == TaskType.GLITCH_DETECTION:
            return "yes"
        elif sample.task_type == TaskType.VISUAL_QA:
            return "A"  # MCQ placeholder
        elif sample.answer_format:
            from benchmarks.loaders.base import AnswerFormat

            if sample.answer_format == AnswerFormat.BINARY:
                return "yes"
            elif sample.answer_format == AnswerFormat.MCQ:
                return "A"
            elif sample.answer_format == AnswerFormat.FREE_TEXT:
                return "The player is exploring the game world."

        return str(sample.ground_truth) if sample.ground_truth else "unknown"
