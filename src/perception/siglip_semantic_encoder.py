...
class SigLIPSemanticEncoder:
    ...
    def encode_batch(
        ...
        This is optimized for throughput - collects multiple images and
        processes them together to maximize GPU utilization.
        
        Args:
            images: List of PIL Images to encode
            batch_size: Override batch size (uses config.batch_size if None)
        ...
        for i in range(0, len(images), batch_size):
            ...
            if self.encoder._processor is not None:
                # Use official processor for proper preprocessing
                inputs = self.encoder._processor(
                    images=batch_images,
                    return_tensors="pt",
                ...
                with torch.no_grad(), torch.autocast(
                    ...
                    embeddings = self.encoder._model.get_image_features(**inputs)
            else:
                # Fallback for placeholder
                tensors = []
                for img in batch_images:
                    tensor, _ = self.region_extractor.prepare_region_tensor(img)
                ...
                _, embeddings = self.encoder(batch)
            
            # Add each embedding to results
            for j in range(embeddings.shape[0]):
                all_embeddings.append(embeddings[j].cpu())
        
        logger.debug(f"Batch encoded {len(all_embeddings)} images in {(len(images) + batch_size - 1) // batch_size} batches")
